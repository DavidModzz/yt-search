...
function _getAcceptLanguage(options) {
  options = options || {};
  var customAcceptLanguage = options.acceptLanguage || options.accept_language;
  if (typeof customAcceptLanguage === 'string') {
    var value = customAcceptLanguage.trim();
    if (value) return value;
  }
  if (Array.isArray(customAcceptLanguage)) {
    var values = customAcceptLanguage.map(function (value) {
  ...
  var hl = String(options.hl || 'en').trim();
  var gl = String(options.gl || 'US').trim();
  var locale = hl;
  if (hl.indexOf('-') >= 0 || hl.indexOf('_') >= 0) {
    locale = hl.replace('_', '-');
  } else if (gl) {
    locale = "".concat(hl, "-").concat(gl);
  }
  if (!locale) {
    locale = 'en-US';
  }
  var baseLanguage = locale.split(/[-_]/)[0] || 'en';
  return "".concat(locale, ",").concat(baseLanguage, ";q=0.9,en-US;q=0.8,en;q=0.7");
}
  ...
  for (var _i = 0; _i < items.length; _i++) {
    ...
    try {
      switch (type) {
        ...
        case 'list':
          {
            var _thumbnail = _normalizeThumbnail(_jp.value(item, '$..primaryThumbnail..url')) || _normalizeThumbnail(_jp.value(item, '$..thumbnail..url')) ||
            // DEPRECATED?
            _normalizeThumbnail(_jp.value(item, '$..thumbnails..url')) ||
            // DEPRECATED
            _normalizeThumbnail(_jp.value(item, '$..thumbnails')) // DEPRECATED?
;
            var _title = _jp.value(item, '$..metadata..title..content') || _jp.value(item, '$..title..text') ||
            // DEPRECATED?
            _jp.value(item, '$..title..simpleText') // DEPRECATED?
;
            var _author_name = _jp.value(item, '$..metadataParts[0]..text..content') || _jp.value(item, '$..shortBylineText..text') ||
            // DEPRECATED?
            ...
            || 'YouTube';
            var _author_url = _jp.value(item, '$..metadataParts[0]..url') || _jp.value(item, '$..shortBylineText..url') ||
            // DEPRECATED?
